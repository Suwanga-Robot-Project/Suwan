...
# [진단] 문제 해결 후 전부 원복할 것
# =====================================================
DIAG_BYPASS_MUX = {6, 11}  # mux6=왼팔6번, mux11=오른팔3번 → 중앙값 필터 우회
DIAG_TRACE_LEFT = {5, 6}  # 왼팔 6번 관절, 7번 그리퍼
DIAG_TRACE_RIGHT = {2}  # 오른팔 3번 관절
# =====================================================
# [공통] FSM 상태 정의
...
REVERSE_CHANNELS_RIGHT = [0, 2, 3, 4, 5, 6]  # 9, 12, 13, 14, 15번 반전
    ...
    for i in range(7):
        ...
        if prev_ticks_left[i] is not None:
            ...
            if i in DIAG_TRACE_LEFT:
                ...
                print(
                    f"  [L{i+1}] raw={raw:5d} ema={ema_values_left[i]:7.1f} "
                    f"tick={tick:5d} prev={prev_ticks_left[i]:5d} diff={diff:4d} "
    ...
    for i in range(7):
        ...
        if prev_ticks_right[i] is not None:
            ...
            if i in DIAG_TRACE_RIGHT:
                ...
                _dfa = abs(tick - _a) if _a is not None else -1
                print(
                    f"  [R{i+1}] raw={raw:5d} ema={ema_values_right[i]:7.1f} "
                    f"tick={tick:5d} prev={prev_ticks_right[i]:5d} diff={diff:4d} "
