...
# [진단] 문제 해결 후 전부 원복할 것
# =====================================================
DIAG_BYPASS_MUX = set()  # 센서 교체 완료 — 우회 불필요
DIAG_TRACE_LEFT = {5, 6}  # 왼팔 6번 관절, 7번 그리퍼
DIAG_TRACE_RIGHT = {2, 3}  # 3번(고장) + 4번(정상, 비교용)
DIAG_R3_NO_ACCEL = True  # [코드3] 오른팔 3번만 가속도 제한 해제
    ...
    while running:
        try:
            ...
            while True:
                ...
                mux_adc_unfiltered = list(mux_adc_vals)
                mux_adc_vals = adc_filter.apply(mux_adc_vals)
                # [진단] 지정 채널만 필터 결과 버리고 원시값으로 되돌림
                for _ch in DIAG_BYPASS_MUX:
                    mux_adc_vals[_ch] = mux_adc_unfiltered[_ch]
...
def process_left_arm(portHandler, packetHandler):
    ...
    for i in range(7):
        ...
        if raw >= FLOATING_THRESHOLD:
            if i in DIAG_TRACE_LEFT:
                print(
                    f"  [L{i+1}] raw={raw} >= FLOATING({FLOATING_THRESHOLD}) → 채널 스킵"
                )
            continue
        ...
        _ema_pre_L = ema_values_left[i]  # [진단] 덮어쓰기 전 순수 EMA 값 보관
        ...
        _tick_pre_L = tick  # [진단] 속도/가속도 제한 걸리기 전 목표값
        ...
        if prev_ticks_left[i] is not None:
            ...
            # [진단] raw → ema → tick → 데드존 상태 한 줄 추적
            if i in DIAG_TRACE_LEFT:
                _a = dead_zone_anchor_left[i]
                _dfa = abs(tick - _a) if _a is not None else -1
                print(
                    f"  [L{i+1}] raw={raw:5d} ema={_ema_pre_L:7.1f} "
                    f"tick_pre={_tick_pre_L:5d} tick={tick:5d} "
                    f"prev={prev_ticks_left[i]:5d} diff={diff:4d} "
                    f"dz={int(in_dead_zone_left[i])} d_anchor={_dfa:4d} "
                    f"(en={dz_enter}/ex={dz_exit})"
                )
...
def process_right_arm(portHandler, packetHandler):
    ...
    for i in range(7):
        ...
        _ema_pre = ema_values_right[i]  # [진단] 덮어쓰기 전 순수 EMA 값 보관
        ...
        _tick_pre = tick  # [진단] 속도/가속도 제한 걸리기 전 목표값
        ...
        if prev_ticks_right[i] is not None:
            ...
            _acc_lim = 9999 if (DIAG_R3_NO_ACCEL and i == 2) else MAX_ACCEL_RIGHT
            if accel > _acc_lim:
                actual_delta = prev_delta_right[i] + _acc_lim
            ...
            if i in DIAG_TRACE_RIGHT:
                _a = dead_zone_anchor_right[i]
                _dfa = abs(tick - _a) if _a is not None else -1
                print(
                    f"  [R{i+1}] raw={raw:5d} ema={_ema_pre:7.1f} "
                    f"tick_pre={_tick_pre:5d} tick={tick:5d} "
                    f"prev={prev_ticks_right[i]:5d} diff={diff:4d} "
                    f"dz={int(in_dead_zone_right[i])} d_anchor={_dfa:4d} "
                    f"(en={DEAD_ZONE_ENTER_RIGHT}/ex={DEAD_ZONE_EXIT_RIGHT})"
                )
